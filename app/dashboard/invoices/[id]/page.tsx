...
export default function InvoiceDetailPage() {
  ...
  const handleDownloadPackingList = async () => {
    ...
    try {
      ...
      const shipAddr = (invoice.customer as any).shippingAddress || invoice.customer.billingAddress;
      if (shipAddr) {
      ...
      // Calculate total units / quantity (NO PRICE CALCULATIONS)
      ...
      pdfContainer.innerHTML = `
          ...
          <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; border-bottom: 2px solid #000; padding-bottom: 15px;">
            <div style="flex: 1;">
              ...
              <div style="font-size: 13px; line-height: 1.5; color: #333;">
                <div style="font-weight: bold;">${invoice.organization.name}</div>
                ${invoice.organization.settings?.email ? `<div>${invoice.organization.settings.email}</div>` : ""}
                ${invoice.organization.settings?.phone ? `<div>${invoice.organization.settings.phone}</div>` : ""}
                ${orgAddressLines.map((line) => `<div>${line}</div>`).join("")}
              </div>
            ...
            <div style="flex: 1; text-align: right; font-size: 13px; line-height: 1.6;">
              <div style="font-size: 15px; font-weight: bold; margin-bottom: 6px;">Ref: PL-${invoice.number.replace(/^INV-/, '')}</div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 25px;">
          ...
          <tfoot>
            <tr style="border-top: 2px solid #111827; background: #f3f4f6;">
              <td colspan="2" style="padding: 10px 12px; font-weight: bold; font-size: 13px;">Total Units / Quantity:</td>
              <td style="text-align: right; padding: 10px 12px; font-weight: bold; font-size: 14px;">${totalUnits.toLocaleString()}</td>
              <td></td>
            </tr>
          </tfoot>
        </table>
